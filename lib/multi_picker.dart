...
class MultiPicker extends StatefulWidget {
  ...
  MultiPicker({Key key, String title, String confirmText, @required this.children, this.onConfirm}) :
...
class _MultiPickerState extends State<MultiPicker> {
  ...
  void initShowList(){
    ...
    if(selectedRows.length == 0 && widget.children.length > 0 && widget.children.first.length > 0){
  ...
  void onChangePicker(int position, int index){
    ...
    if(index == 0){
      selectedRows[index] = {pickerTree.root[position].k: pickerTree.root[position].v};
    }
    else{
  ...
  Iterable<Widget> _renderPicker(){
    ...
    for(int i = 0; i < widget.children.length; i++){
      list.add(Expanded(
        child: CupertinoPicker(itemExtent: 45,
            ...
            child: Row(
              ...
              children: <Widget>[
                ...
                GestureDetector(
                  ...
                  child: Container(
                      height: 45,
                      child: Center(child: Text(widget.confirmText, style: TextStyle(color: Theme.of(context).primaryColor, fontWeight: FontWeight.bold, fontSize: 15)))
                  ),
                ),
              ],
            ),
